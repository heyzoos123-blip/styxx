# -*- coding: utf-8 -*-
"""Baseline-015 — Pythia-410M per-token log-probability detector.

Tests whether inverse-scaling generalizes across model families. Same
algorithm as Baselines 011-014. EleutherAI/pythia-410m is 2.6x the size
of Pythia-160M (Baseline-014, which gave 2/4 with D2 axis clean).
"""
...
def _ensure_model():
    ...
    try:
        ...
        raise ImportError(
            f"baseline_015 requires transformers + torch. install: "
            f"pip install transformers torch. original: {e}"
        )
    _TOKENIZER = AutoTokenizer.from_pretrained("EleutherAI/pythia-410m")
    _MODEL = AutoModelForCausalLM.from_pretrained("EleutherAI/pythia-410m")
    _MODEL.eval()


def detect(question: str, response: str) -> Dict[str, float]:
    """Mean per-token log-probability of response tokens under pythia-410m."""
    import torch
    _ensure_model()
